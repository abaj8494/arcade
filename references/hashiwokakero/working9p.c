...
bool check_solved(Puzzle p) {
  for (int i = 0; i < p->nislands; i++) {
    ...
    printf("failed %d\nhas %d\nmax %d\n", i, curr->curr_bridges, curr->max_bridges);
    return false;
  }
  return true;
}
...
bool forward_check(Puzzle p) {
  for(int i = 0; i < p->nislands; i++) {
    ...
    for(int dir = 0; dir < DIRECTIONS; dir++) {
      if(can_build_bridge(p, p->nodes[i], dir)) buildable++;
    }
    if(buildable == 0 && p->solved[i] == 0) return false;
  }
  return true;
}
...
void heuristics(Puzzle p) { //this function is not updating the islands current->bridge count correctly!
  ...
  //printf("\n");
  // for (int i = 0; i < p->nislands; i++) {
  //   printf("%.2d ", p->nodes[i]->max_bridges);
  // }
  // printf("\n");
  // for (int j = 0; j < p->nislands; j++) {
  //   printf("%.2d ", p->nodes[j]->curr_bridges);
  // }
  // printf("\n");
  for (int i = 0; i < p->nislands; i++) {
    Island curr = p->nodes[i];
    if(curr->curr_bridges == curr->max_bridges) {
      p->solved[i]=1;
    }
  }
  // printf("\n");
...
int solve_map(Puzzle p) { // backtrack
    ...
    if(check_solved(p)) return TRUE;
    //if(forward_check(p) == false) return FALSE;

    for(int i = 0; i < p->nislands; i++) {
        //printf("island %d\n",i);
        Island curr = p->nodes[i];
        if(curr->max_bridges == curr->curr_bridges) p->solved[i] = 1;
        else p->solved[i] = 0;
        for(int dir = 0; dir < DIRECTIONS; dir++) {
            int built = 0;
            ...
            for(int j = 0; j < built; j++) remove_bridge(p,curr,dir);
        }
    }
    return FALSE;
}
